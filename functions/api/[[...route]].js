...
function ensureDefaultSources2(db) {
  ...
  upsertDefaults(getDefaultSources2());
  db.prepare(`
    UPDATE sources
    SET enabled = 0
...
var repositories = {
  ...
  keywords: {
    ...
    create(term, scope) {
      const account = detectAccountInfo2(term.trim());
    ...
    update(id, input) {
      ...
      const detected = input.term ? detectAccountInfo2(nextTerm) : null;
      getDb().prepare(`
        UPDATE keywords
  ...
  sources: {
    ...
    byId(id) {
      ...
      return row ? mapSource2(row) : null;
    }
  },
  items: {
    list(limit = 80) {
  ...
  scanRuns: {
    ...
    last() {
      ...
      return row ? mapScanRun2(row) : null;
    }
  }
};
...
function mapSource2(row) {
  return {
    ...
    providerType: parseProviderType2(row.provider_type),
    reliabilityTier: parseReliabilityTier2(row.reliability_tier),
...
function mergeItemEvidence(db, itemId, input) {
  ...
  `).run({
    ...
    domain: hostname3(input.normalizedUrl)
  });
...
async function runScan() {
  ...
  scanning = true;
  const scanRunId = await repos2.scanRuns.start();
  const totals = { fetched: 0, inserted: 0, evaluated: 0 };
  const isVercel = Boolean(process.env.VERCEL);
  try {
    const keywords = await repos2.keywords.active();
    let sources = await repos2.sources.active();
    if (isVercel) {
    ...
    for (const raw of deduped) {
      ...
      const result = await repos2.items.insert(raw);
      if (!result) continue;
      if (result.inserted) {
        totals.inserted += 1;
    ...
    for (const itemId of insertedIds) {
      const item = await repos2.items.byId(itemId);
      if (!item) continue;
    ...
    const existingTop = (await repos2.items.list(50)).filter((i) => !insertedIds.includes(i.id)).slice(0, 8);
    ...
    for (const candidate of aiCandidates) {
      const item = await repos2.items.byId(candidate.id);
      if (!item) continue;
      if (!isKeywordMentioned(item.title, item.summary, item.matchedKeyword)) {
        ...
        await repos2.items.updateStatus(candidate.id, "ignored");
        continue;
      }
      ...
      const settings = await repos2.settings.all();
      const evaluation = await evaluateItem(item, keyword, source, settings);
      await repos2.items.addEvaluation(candidate.id, evaluation);
      totals.evaluated += 1;
    }
    for (const candidate of scoredItems) {
      ...
      await repos2.items.updatePriority(candidate.id, priorityScore, finalscore, status);
    }
    await repos2.scanRuns.finish(scanRunId, "success", totals);
    const archivedCount = await repos2.items.archiveStaleItems();
    if (archivedCount > 0) {
    ...
    const message = error instanceof Error ? error.message : String(error);
    await repos2.scanRuns.finish(scanRunId, "failed", { ...totals, error: message });
    throw error;
  } finally {
    scanning = false;
  }
