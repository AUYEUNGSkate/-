...
export const tursoRepos = {
  ...
  items: {
    ...
    async updatePriority(id:number,priorityScore:number,freshnessScore:number,status:string){await run("UPDATE items SET priority_score=?,freshness_score=?,status=? WHERE id=?",[priorityScore,freshnessScore,status,id]);},
