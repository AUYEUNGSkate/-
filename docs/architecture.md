...
热点页采用“克制情报台”展示结构，保持游戏行业情报雷达气质，但降低装饰感和卡片堆叠：

- 顶部保留品牌、视图切换、Aceternity 流光扫描按钮、未读入口和刷新入口。
- 首屏概览展示总热点、今日新增、紧急热点和启用监控词，并用简短说明帮助判断状态。
- 主列为实时热点流，包含关键词追踪、搜索/排序/已读/重要程度/来源筛选条、热点列表和归档入口。
- 右侧为 AI 情报栏，包含 AI 简报、当前筛选摘要、未读/高优先级/今日新增指标和 Top 高优先级热点。
- 热点卡片采用左侧优先级/热度轨道 + 右侧正文结构，突出标题、来源、发布时间、互动数据和 AI 分析理由。
- 小屏幕下保持单栏，顺序为导航、扫描、概览指标、关键词、筛选、热点流，再进入 AI 情报栏，避免右侧栏抢占首屏。
- 热点卡片保留标题打开原文并自动标记已读的行为；AI 分析理由仅消费现有 `evaluation` 数据，不新增接口字段。
...
- 深色工作台背景，减少大面积青蓝铺色，保持长期阅读舒适度。
- Aceternity UI 只用于关键焦点：`Spotlight` 提供低强度背景光，`HoverBorderGradient` 用于“立即扫描”按钮。
- 扫描按钮保持图标、文字和移动端宽度稳定，不因流光边框造成挤压。
- 扫描线、热度光谱、未读强调等动效保持轻量，并尊重 `prefers-reduced-motion`。
- 不做普通卡片堆叠后台，热点流优先服务快速扫描、判断和打开原文。
- 页面首屏直接是可操作工作台，不做营销式落地页。
- 移动端保留核心操作：立即扫描、查看概览、筛选热点、打开原文。
